...
public class BattleServiceImp implements BattleService {
    ...
    public BattleInitiativeResultResponse diceInitiative(InteractInBattleDto dto) {
        ...
        var turn = new Turn(null, 1L, null, null, battle);
    ...
    public BattleStatusResponse attack(InteractInBattleDto dto) {
        ...
        this.doTurnValidations(battle, actualTurn, new ValidatePlayerTurnAttack());
    ...
    private BattleStatusResponse attack(Battle battle) {
        var actualTurn = this.turnService.getActualTurn(battle);
        if (actualTurn.getDamageDealt() != null && actualTurn.getDamageReceived() != null){
            var turn = new Turn(null, actualTurn.getTurnNumber(), null, null, battle);
            battle.getTurns().add(turn);
            actualTurn = this.turnService.createTurn(turn, battle);
        }
        int damageDealt = 0;
        AttackValidations attack = new TryToAttack(battle.getHero(), battle.getMonster());
        if (attack.validate()) {
            DamageActions damage = new CalculateDamage(battle.getHero());
            damageDealt = damage.perform();
        }
        actualTurn.setDamageDealt(damageDealt);
        battle = this.battleRepository.save(battle);
        return new BattleStatusResponse(actualTurn.getTurnNumber(), battle, "Defenda-se");
    }

    private BattleStatusResponse defend(Battle battle) {
        return null;
    }
}
